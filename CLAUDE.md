...
- All user-visible wording and formatting lives in src/ccdrift/texts.py, so the check, report and status line stay consistent and the import stays light. Option help is the exception: it stays beside its option in cli.py. tests/test_command_texts.py lists the modules held to this. Formats other programs read or write are exempt: the module constants in schedule.py (crontab marker, unit files), page.py (CSS), notify.py (AppleScript) and logs.py (Claude Code's error banners), and page.py's markup.
