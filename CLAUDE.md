...
- Tests: `uv run --group dev --group lab pytest` (811 tests, ~45s). `testpaths = ["tests", "lab"]`, so a bare `pytest` runs both suites.
...
- **Bump `PARSER_VERSION`** (history.py) whenever `parse_file` output changes, and extend its numbered comment list. Without the bump, `History.update` skips unchanged transcripts and the store keeps stale rows.
...
- **`SCHEMA_VERSION`** (history.py): a new column needs both an additive `ALTER TABLE` branch in `_migrate` and an edit to `SCHEMA`. The version check runs before any DDL on purpose; a store written by a newer ccdrift is refused, not migrated.
