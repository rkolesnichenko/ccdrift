...
- All user-visible wording and formatting lives in src/ccdrift/texts.py, so the check, report and status line stay consistent and the import stays light. Option help is the exception: it stays beside its option in cli.py. tests/test_command_texts.py holds cli.py, status.py, report.py, spend.py, schedule.py, draft.py and the rule modules whose report sections moved (settings, sessions, hooks, failures) to this; schedule.py's module constants are the schedulers' own formats and are exempt.
