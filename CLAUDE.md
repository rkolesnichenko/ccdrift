...
- Tests: `uv run --group dev --group lab pytest` (932 tests, ~60s). `testpaths = ["tests", "lab"]`, so a bare `pytest` runs both suites.
...
- All user-visible wording and formatting lives in src/ccdrift/texts.py, so the check, report and status line stay consistent and the import stays light. Option help is the exception: it stays beside its option in cli.py. tests/test_command_texts.py holds every module to this: cli.py and history.py in what they print, raise or return (option help and SQL stay where they are), every other module to having no words outside a docstring, including one added later. Text other programs read or write is exempt: the module constants in schedule.py (crontab marker, unit files), page.py (CSS), notify.py (AppleScript), logs.py (Claude Code's error banners) and changelog.py (the words release notes are searched for), and page.py's markup.
...
- **Bump `PARSER_VERSION`** (history.py) whenever `parse_file` output changes, and extend its numbered comment list. Without the bump, `History.update` skips unchanged transcripts and the store keeps stale rows. tests/test_parser_version.py pins a digest of `parse_file`'s output on a fixture holding every record kind to the version, so output that moves without a bump fails; put the new version and digest in `PINNED` with the bump.
- **Log-format drift goes in exactly one place**: the `CANDIDATES` dict in logs.py. Add a new field variant there, never parse it ad hoc. That is a parser change, so bump `PARSER_VERSION` as well.
- **`SCHEMA_VERSION`** (history.py): a new column needs both an additive `ALTER TABLE` branch in `_migrate` and an edit to `SCHEMA`. The version check runs before any DDL on purpose; a store written by a newer ccdrift is refused, not migrated. tests/test_schema_versions.py keeps each version's store as `tests/fixtures/schema-v<N>.sql`: a `SCHEMA` edit without a bump fails, and a bump needs the new store dumped to a fixture of its own, after which every older one must migrate to it.
- **`STATE_VERSION` and `CONTEXT_RULE`** (state.py): changing the session-start judging rule requires bumping `CONTEXT_RULE` so existing states are re-judged once. A state from a newer ccdrift is never overwritten.
- **The status line stays import-light**: `ccdrift status --short` runs on every status-line refresh and must import neither pandas nor numpy (pandas alone costs ~0.3s). cli.py defers heavy imports into the command branch and imports only state and texts at module level. Guarded by `test_status_short_loads_neither_pandas_nor_numpy` in tests/test_status.py.
- **Privacy**: `report --json`, `cost --json` and `incident draft` emit aggregates only, no paths, session ids or project names; `cost --json` lists the project and branch dimensions under `withheld`. `report --html` is the exception and is not shareable: it names project folders exactly as the terminal report does, and the page's own last line says so (`test_the_page_holds_the_rule_both_charts_the_table_and_the_footer_and_no_script` in tests/test_page.py). `peek` prints text, ids and paths as their length only, shows a content block as its type and the size of the rest, and deliberately omits the transcript path because it names the project folder. Owner-only 0600: history through `make_private`, state through the temporary file `save_state` writes it by; the log at `schedule install`, and again on each check through `make_stream_private` when the check's stdout is that log; the `--html` page as it is written.
