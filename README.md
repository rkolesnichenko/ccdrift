...
| **ccdrift: session start changed** | Sessions start with at least 25% more or less context than the 10 before them, on 3 in a row. Each session is measured against its own project's recent level, so moving between projects is not a change; the alert says whether every project moved (Claude Code or your global config) or one did (that project's CLAUDE.md, MCP servers or skills). | `ccdrift report` names each project's typical session start; `ccdrift report --by version` compares versions. |
